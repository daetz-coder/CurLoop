...
if not exist "%~dp0..\dist\loop.js" (
  echo [fail] 未找到 Harness 编译产物（dist\loop.js）。
  echo        请从 D:\2026AppDev\CursorHarness\unattended\run_unattended.bat 原位置运行，
  echo        不要复制到其他目录；或改用 run_here.bat / run_limit_sim.bat。
  echo        若尚未构建：cd /d D:\2026AppDev\CursorHarness ^&^& npm run build
  pause
  exit /b 1
)
cd /d "%~dp0.."
rem 管理员判定与 loop 的 isAdmin 同源
...
node bin\curloop.js --mode %MODE% %PROJECT_ARG% %3 %4 %5
set "RC=!errorlevel!"
if "!RC!"=="0" goto :done
if "!RC!"=="2" goto :done
if "!RC!"=="130" goto :done
